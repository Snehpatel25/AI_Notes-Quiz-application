...
export const createQuiz = async (userId, subject, gradeLevel, topic, numQuestions = 10) => {
    ...
    if (USE_MOCK_DB) {
        ...
        const performanceResult = await pool.query('SELECT * FROM user_performance WHERE user_id = $1 AND subject = $2 AND grade_level = $3', [userId, subject, gradeLevel]);
        if (performanceResult.rows.length > 0) {
            const performance = performanceResult.rows[0];
            difficultyDistribution = calculateDifficultyDistribution(parseFloat(performance.average_score), performance.total_quizzes);
        }
        else {
    ...
    if (USE_MOCK_DB) {
        ...
        const result = await pool.query(`INSERT INTO quizzes (user_id, title, grade_level, subject, questions, difficulty_distribution)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [
            userId,
            quizData.title,
...
export const submitQuiz = async (submission) => {
    ...
    if (USE_MOCK_DB) {
        ...
        const submissionResult = await pool.query(`INSERT INTO quiz_submissions (quiz_id, user_id, answers, score, total_questions, percentage_score, improvement_tips)
    ...
    if (USE_MOCK_DB) {
        ...
        await pool.query(`INSERT INTO user_performance (user_id, subject, grade_level, total_quizzes, total_score, average_score, difficulty_level, last_quiz_date)
       ...
       DO UPDATE SET
         ...
         last_quiz_date = NOW(),
         updated_at = NOW()`, [submission.userId, subject, grade_level, score, percentageScore, percentageScore >= 80 ? 'hard' : percentageScore >= 60 ? 'medium' : 'easy']);
    }
    return {
        id: submissionRow.id,
        score,
...
export const getQuizHistory = async (userId, filters) => {
    if (USE_MOCK_DB) {
        ...
        if (filters.gradeLevel)
            rows = rows.filter((r) => r.grade_level === filters.gradeLevel);
        if (filters.minScore !== undefined)
            rows = rows.filter((r) => r.percentage_score >= filters.minScore);
        if (filters.maxScore !== undefined)
            rows = rows.filter((r) => r.percentage_score <= filters.maxScore);
        if (filters.fromDate)
            rows = rows.filter((r) => new Date(r.completed_at) >= new Date(filters.fromDate));
        if (filters.toDate)
            rows = rows.filter((r) => new Date(r.completed_at) <= new Date(filters.toDate));
    ...
    `;
        ...
        if (filters.minScore !== undefined) {
            query += ` AND qs.percentage_score >= $${paramIndex}`;
        ...
        if (filters.fromDate) {
            query += ` AND qs.completed_at >= $${paramIndex}`;
...
export const getHint = async (question, subject) => {
