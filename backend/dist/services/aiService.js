...
export const generateQuiz = async (subject, gradeLevel, topic, numQuestions = 10, difficultyDistribution) => {
...
export const generateHint = async (question, subject) => {
...
export const generateImprovementTips = async (mistakes, subject) => {
...
Provide exactly 2 tips, each on a new line, without numbering.`;
    try {
        ...
        return tips.split('\n').filter((tip) => tip.trim().length > 0).slice(0, 2);
    }
    catch (error) {
...
export const calculateDifficultyDistribution = (averageScore, totalQuizzes) => {
